...
from homeassistant.components.light import (                                           #继承HASS官方组件里的light
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    # from pykonkeio.manager import get_device_info    
    # device_info = await get_device_info(host)     

    entity = KonkeLight(device, name, model, device_info)
    async_add_entities([entity])

class KonkeLight(Light):
    """Konke light device."""

    def __init__(self, device, name: str, model: str, device_info):
    ...
    def unique_id(self) -> str:
        ...
        if self._mac and self._model == MODEL_K2_LIGHT:
            # return self._device.mac + ':light'
            return self._mac + ':light'
        else:
