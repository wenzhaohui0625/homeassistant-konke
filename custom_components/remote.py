...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Optional(CONF_TYPE, default=TYPE_IR): vol.In((TYPE_IR, TYPE_RF, TYPE_RT, TYPE_TP)),
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    async def async_service_handler(service):
        ...
        for _entity in ENTITIES:
            if _entity.entity_id == entity_id:
                entity = _entity      
        ...
        _LOGGER.debug('Start learning %s remote on slot %s', entity.type, slot)
...
class KonkeRemote(RemoteDevice):
    ...
    def unique_id(self) -> str:
        ...
        return '%s:%s' % (self._mac, self.type)
    ...
    def available(self) -> bool:
        ...
        elif self._type == TYPE_RT:
            return self._device.is_support_rt
        elif self._type == TYPE_TP:
            return self._device.is_support_tp
        else:
            return False
    ...
    async def async_update(self):
        ...
        try:
            ...
            if prev_available:
                ...
                # _LOGGER.warning('Device is offline %s：%s', self._mac, self._type)
